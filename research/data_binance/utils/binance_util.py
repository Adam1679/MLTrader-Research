...
from vnpy.trader.constant import Exchange, LongShortType
from vnpy.trader.object import LongShortData
from vnpy_binance.binance_usdt_gateway import generate_datetime
@dataclass
...
try:
    ...
    for asset_info in info["symbols"]:
        ...
        UM_ASSET_INFO[asset_info["symbol"]] = BinanceUmContractInfo(asset_name, 
                                                                asset_info["status"],
...
def _query_ls_history_help(symbol, path, type, date):
    history: List[LongShortData] = []
    limit: int = 500
    start: int = int(date.timestamp())
    end: int = int((date + timedelta(days=1)).timestamp())
    # 创建查询参数
    params: dict = {
        "symbol": symbol,
        "period": "5m",
    ...
    resp = requests.get(F_REST_HOST + path, params=params)

    # 如果请求失败则终止循环
    if resp.status_code // 100 != 2:
        msg: str = f"query historical data failed, status_code={resp.status_code}, msg={resp.text}"
        logging.error(msg)
        return []
    else:
        data: dict = resp.json()
        if not data:
            msg: str = f"query historical data but got None"
            logging.error(msg)
            return []

        buf: List[LongShortData] = []

        for row in data:
            bar: LongShortData = LongShortData(
                gateway_name="",
                symbol=symbol,
        ...
        history.extend(buf)
        msg: str = f"query historical data succeed, {symbol}, {buf[0].datetime} - {buf[-1].datetime}"
        print(msg)

    return history


def query_ls_history(symbol, folder):
    today = datetime.utcnow().replace(minute=0, microsecond=0, hour=0)

    for day in range(30):
        history = []
        date_ = today - timedelta(days=day)
        path = Path(folder).joinpath("futures").joinpath("ls").joinpath(symbol)
        path.mkdir(parents=True, exist_ok=True)
        file_path = path.joinpath(
            "{}-5m-ls-{}".format(symbol, date_.strftime("%Y-%m-%d"))
        )
        if file_path.exists():
            print(str(file_path) + " exists!")
            continue
        history.extend(
            _query_ls_history_help(
                symbol,
                "/futures/data/globalLongShortAccountRatio",
                type=LongShortType.GlobalPosition,
                date=date_,
            )
        )
        history.extend(
        ...
        history.extend(
            _query_ls_history_help(
                ...
                type=LongShortType.TopPosition,
                date=date_,
            )
        )
        if len(history) > 0:

            pd.DataFrame(history).to_csv(file_path, index=False, header=False)
        else:
            break
