...
from download_binance import download_5m_ls_data

root = Path("/Users/bytedance/binance_data")
binance_data_root = Path("/Users/bytedance/binance_data/data")
...
def get_symbolsV2(symbol_type="future"):
    ...
    if symbol_type == "future":
        df = pd.read_csv(
            root / "perpetual_symbol_info_selected.csv", index_col="symbol"
        )
...
def download_futures(symbols, trades=True):
    ...
    if trades:
        cmd = f"python research/data_binance/download_binance_trades.py -s {f_s} -folder /Users/bytedance/binance_data -t um -trades"
    else:
        cmd = f"python research/data_binance/download_binance.py -s {f_s} -folder /Users/bytedance/binance_data -t um -i 1m"
    print(cmd)
    if os.system(cmd) < 0:
...
def download_spots(symbols):
    f_s = " ".join(symbols)
    print(f"download data for {f_s}")
    cmd = f"python research/data_binance/download_binance.py -s {f_s} -folder /Users/bytedance/binance_data -t spot -i 1m"
    print(cmd)
    if os.system(cmd) < 0:
...
def delete_duplicate_data(daily_binance_data_root, monthly_binance_data_root):
    ...
    for symbol in all_spot_symbols:
        ...
        for i in range(len(all_dates) - 1):
            ...
            ).total_seconds() == 60 * 24 * 60, all_dates
...
if __name__ == "__main__":
    # ---- * ---- macro variables
    from research.data_binance.utils.binance_util import S_DEFAULTS
    ...
    # )
    # os.system("bash research/data_binance/concat_all_zip_files.bash")
    ...
    os.system("bash research/data_binance/utils/concat_all_zip_files.bash")
    # print("beging faking data...")
